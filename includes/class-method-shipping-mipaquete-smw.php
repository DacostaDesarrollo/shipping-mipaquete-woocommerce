...
class WC_Shipping_Method_Shipping_Mipaquete_SMW extends WC_Shipping_Method
{
    ...
    {
        global $woocommerce, $wp, $WCFM, $WCFMmp;
        ...
        /**
         * $vendor_data Guarda los datos del vendedor
         * @var Array
         */
        ...
        if (!empty($package)) {
            ...
            //TODO: verificar $this->address_sender si hay que reemplazarlo en esta parte 
        
            $vendor_data = get_user_meta( $package["vendor_id"], 'wcfmmp_profile_settings', true );
            $city_origin = $vendor_data["address"]['city'];
            $state_origin = $vendor_data["address"]['state'];
            $country_origin = $vendor_data["address"]['country'];
            
            if($country_origin !== 'CO' || empty($state_origin)){

                return apply_filters( 'woocommerce_shipping_' . $this->id . '_is_available', false, $package, $this );
            }
            $name_state_origin = Shipping_Shipping_Mipaquete_SMW::name_destination($country_origin, $state_origin);
            
            if (empty($name_state_origin)){

                return apply_filters( 'woocommerce_shipping_' . $this->id . '_is_available', false, $package, $this );
            }

            $address_origin = "$city_origin - $name_state_origin";
            $origin = array_search($address_origin, $cities);

            $this->city_sender = $origin;
